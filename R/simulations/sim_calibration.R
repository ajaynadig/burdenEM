...
if (!exists("posterior_expectation2", mode = "function")) {
  source(file.path(dirname(sys.frame(1)$ofile %||% "."), "..", "estimation", "model.R"))
}
...
calculate_calibration_metrics <- function(genes_df,
  ...
  # If model has to_per_allele_effects method, use it; otherwise use identity
  has_per_allele_fn <- !is.null(model$to_per_allele_effects) && is.function(model$to_per_allele_effects)

  effect_size_fn <- if (per_allele_effects && has_per_allele_fn) {
    function(x, row) model$to_per_allele_effects(row, x)
  } else if (per_allele_effects) {
    # No per-allele conversion available, use identity
    function(x, row) x
  } else if (has_per_allele_fn) {
    function(x, row) model$to_per_allele_effects(row, x) * sqrt(row$burden_score)
  } else {
    # For grid-based models, x is already the effect size
    function(x, row) x
  }
