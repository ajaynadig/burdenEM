...
meta_analyze_heritability <- function(true_results_df, estimated_results_df, discard_outliers = FALSE) {
  ...
  if (nrow(estimated_results_df) == 0) {
    warning("No estimated heritability results available. Returning only true heritability summary.")
    # Return summary of true results only
    true_metric_cols <- names(true_results_df)[sapply(true_results_df, is.numeric)]
    summary_df <- true_results_df %>%
      dplyr::group_by(abbreviation) %>%
      dplyr::summarise(
        dplyr::across(all_of(true_metric_cols), list(true_mean = mean, true_sd = sd), .names = "{.col}_{.fn}"),
        .groups = "drop"
      )
    return(summary_df)
  }

  if (nrow(true_results_df) == 0) {
    warning("No true heritability results available. Returning only estimated heritability summary.")
    estimated_metric_cols <- names(estimated_results_df)[sapply(estimated_results_df, is.numeric)]
    summary_df <- estimated_results_df %>%
      dplyr::group_by(abbreviation) %>%
      dplyr::summarise(
        dplyr::across(all_of(estimated_metric_cols), list(mean = mean, sd = sd), .names = "{.col}_{.fn}"),
        .groups = "drop"
      )
    return(summary_df)
  }
