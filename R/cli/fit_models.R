...
source("R/estimation/run_pipeline.R")
...
option_list = list(
    make_option(c("-a", "--annotation"), type="character", default="all",
        help="Single functional annotation category to process (e.g., 'pLoF', 'missense', 'synonymous'). Use 'all' for simulations or data without annotations. [default: %default]", metavar="character"),
    ...
    make_option(c("-g", "--genes_file"), type="character", default=NULL,
        help="Optional: Path to the genes file template (optionally use <ANNOTATION>, <LOWER>, <UPPER>, and <DATASET> placeholders). Required for LD correction and gene features.", metavar="character"),
    ...
    make_option(c("--intercept_frequency_bin_edges"), type="character", default="0,1e-5,1e-4,1e-3,1",
        help="Comma-separated string of AF bin edges for intercept/overdispersion calculation. Must cover full AF range. [default: %default]", metavar="character"),
...
# Note: --annotation defaults to "all", --genes_file is optional
# Genes file is only required for LD correction or gene features
if (!is.null(opt$feature_col_name) && is.null(opt$genes_file)) {
    stop("When --feature_col_name is specified, --genes_file must also be provided.", call.=FALSE)
}
if (opt$correct_for_ld && is.null(opt$genes_file)) {
    stop("When --correct_for_ld is specified, --genes_file must also be provided.", call.=FALSE)
}
...
process_study_cli <- function(study_row, opt_config, freq_range_cli, icept_freq_bins_cli) {
    ...
    processed_genes_file <- if (!is.null(genes_file_template)) {
        stringr::str_replace(genes_file_template, "<DATASET>", current_study$dataset)
    } else {
        NULL
    }
