...
GNOMAD_LOF_METRICS_FILE = Path(TOP_DIR) / "burdenEM_results/data/utility/gnomad.v2.1.1.lof_metrics.by_gene.txt.bgz"

# Annotations to get from the gnomAD file
GENE_ANNOT_NAMES = ['oe_lof', 'oe_mis_pphen', 'oe_lof_upper', 'cds_length']
...
R_OUTPUT_FILE = Path(TOP_DIR) / "burdenEM_results/data/utility/ukbb_ld_corrected_burden_scores.tsv"
...
def main():
    ...
    if gene_results_list:
        ...
        for category in unique_categories_in_results:
            ...
            output_filename = f"{args.output_prefix}_{category}.tsv"
